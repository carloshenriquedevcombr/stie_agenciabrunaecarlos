document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---------------------------------------------------------
    // 1. FUNÇÃO BLINDADA PARA PEGAR A URL DA HOSTGATOR
    // ---------------------------------------------------------
    function getBaseUrl() {
    ...
    // ---------------------------------------------------------
    // 2. CARREGAR A GALERIA DE IMAGENS (COM BOTÃO DE EXCLUIR)
    // ---------------------------------------------------------
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        data.files.forEach(file => {
                            ...
                            // Monta o card com a imagem, link, botão de copiar e botão vermelho de excluir
                            card.innerHTML = `
    ...
    window.deleteImage = function(filename) {
        // Alerta de confirmação de segurança
        if (!confirm('Tem certeza? Se apagar, os e-mails e campanhas que usam esse link vão quebrar a imagem.')) {
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                loadGallery(); // Se apagou com sucesso, recarrega a galeria
            } else {
    ...
    function fecharModal() {
        ...
        setTimeout(() => { modalImg.src = ''; }, 300); // Limpa a imagem após a animação
    }
    ...
    document.addEventListener('keydown', (e) => { 
        if (e.key === 'Escape' && imageModal.classList.contains('active')) fecharModal(); 
    });

    // ---------------------------------------------------------
    // 6. FUNÇÃO UNIVERSAL PARA COPIAR A URL
    // ---------------------------------------------------------
    btnCopyModal.addEventListener('click', function() { copyToClipboard(this, modalUrlInput); });
